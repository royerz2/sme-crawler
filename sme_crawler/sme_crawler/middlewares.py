...
class SmeCrawlerSpiderMiddleware:
...
class SmeCrawlerDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class DuplicateUrlFilterMiddleware:
    ...
    def from_crawler(cls, crawler):
        ...
        if not mongo_uri or not mongo_database or not mongo_collection:
            # Log a warning or raise an exception if MongoDB settings are missing
            crawler.logger.warning("MongoDB settings not found. DuplicateUrlFilterMiddleware will not be active.")
            return cls(None, None, None) # Return an inactive middleware instance
        
        middleware = cls(mongo_uri, mongo_database, mongo_collection)
    ...
    def spider_opened(self, spider):
        if self.mongo_uri:
            try:
                ...
                spider.logger.info("Connected to MongoDB for duplicate URL filtering.")
    ...
    def process_request(self, request, spider):
        if self.collection:
            url = request.url
            # Check if URL exists in MongoDB collection
